...
aws events put-rule --name $rule --schedule-expression "cron(0 20 * * ? *)" #this currently runs at 12:00 PST (want to have more dynamic and specific input to this for real usage)

aws lambda add-permission --function-name DBtester --statement-id $statement --action 'lambda:InvokeFunction' --principal events.amazonaws.com --source-arn arn:aws:events:us-west-1:392970261554:rule/$rule
...
echo '[{"Id": "1","Arn": "arn:aws:lambda:us-west-1:392970261554:function:DBtester"}]' > targets.json
